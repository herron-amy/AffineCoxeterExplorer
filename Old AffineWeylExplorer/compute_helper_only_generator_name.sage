...
def _a1xa1_plot_grid(bounding_box, H0_alcoves_dict):
    ...
    G += text(r"First Ã₁ $\alpha$", (2.2,0.2), fontsize=10, color='red',
              horizontal_alignment='left', vertical_alignment='top')
    G += text(r"Second Ã₁ $\alpha$", (0,2.25), fontsize=10, color='blue', rotation=90,
...
def _Cocon_Elements_A1xA1(e1, e2, e3, e4, bounding_box):
    ...
    G, tp1, cp_tmp1, cp1 = _a1xa1_chosen_elt(G, e1, e2, bounding_box,
                                              coroot_vertices, B, H0, 'red')
    G, tp2, cp_tmp2, cp2 = _a1xa1_chosen_elt(G, e3, e4, bounding_box,
...
# ============================================================
# Affine rank 2  —  geometry data (alcoves, vertices, keys)
# ============================================================
...
# ============================================================
# Affine rank 2  —  mod set, reflect, finite, eta functions
# ============================================================
...
# ============================================================
# Affine rank 2  —  main computation functions
# ============================================================

def _conjugacy_class_affine2(letter, elt, bounding_box):
    """Conjugacy class for affine rank-2 groups (letter in A, B, C, G)."""
    E = ExtendedAffineWeylGroup([letter,2,1])
    PW0 = E.PW0(); FW = E.FW(); S_ext = FW.simple_reflections()
    L = RootSystem([letter,2,1]).ambient_space()
    W = WeylGroup([letter,2,1]); S_weyl = W.simple_reflections()
    identity = W.one()
    ew = W.one()
    for c in elt: ew = ew * S_weyl[int(c)]
    ...
    G += text(r"$\alpha_1$",(vertex1[0]+e1a,vertex1[1]+e1b),fontsize=14,color='red',
              horizontal_alignment=ha1,vertical_alignment=va1)
    G += text(r"$\alpha_2$",(vertex2[0]+e2a,vertex2[1]+e2b),fontsize=14,color='blue',
    ...
    print(f"Basis B (columns = simple coroots):")
    print(f"  b1 = {_fmt_vec(vertex1)}")
    print(f"  b2 = {_fmt_vec(vertex2)}")
    print(f"Translation vectors are in basis B coordinates (n1, n2) where t = n1*b1 + n2*b2")
    ...
    print(f"Basis B (columns = simple coroots):")
    print(f"  b1 = {[_fmt_val_a2(x) for x in vertex1]}")
    print(f"  b2 = {[_fmt_val_a2(x) for x in vertex2]}")
    print(f"Translation vectors are in basis B coordinates (n1, n2) where t = n1*b1 + n2*b2")
    _cc_aff2 = []
    _seen_aff2 = set()
    for _k in H0dict.keys():
        ...
        for _vec in H0dict[_k].get("reflect", []):
            ...
            _seen_aff2.add(_key)
            _cc_aff2.append(f"t_({_a},{_b}) · {_fc_str}")
    _cc_aff2.sort(key=lambda x:(len(x),x))
...
def _Cocon_Elements_affine2(letter, elt1, elt2, bounding_box):
    """Coconjugation for affine rank-2 groups (letter in A, B, C, G)."""
    E = ExtendedAffineWeylGroup([letter,2,1])
    PW0 = E.PW0(); FW = E.FW(); S_ext = FW.simple_reflections()
    L = RootSystem([letter,2,1]).ambient_space()
...
# ============================================================
# Rank 3  —  shared utilities (B3 and C3 share most code)
# ============================================================

def _r3_get_coroot_vertices_in_B(bounding_box):
    """Coroot lattice points in B-coordinates for rank-3 groups."""
    verts = []
...
def _r3_add_stripes_to_tetrahedron(G, vertices, faces,
                                    stripe_color='lightgray', num_stripes=4, stripe_width=2):
    for face in faces:
...
def _a3_conjugacy_class_plot(W_fin, B, H0dict, bbox, alcove_verts, trans_B, fin_weyl):
    ...
    def _fmt_b(x):
        ...
        f = float(x)
        return str(int(f)) if f == int(f) else f"{f:.10g}"
    print("Basis B (columns = simple coroots of A3):")
    for _i in range(3):
...
def _conjugacy_class_A3(elt, bbox):
    ...
    # Also show what PW0 decomposition gives
    S_FW_dbg = setup['FW'].simple_reflections()
    ee_dbg = S_FW_dbg[int(elt[0])]
    for i in range(1,len(elt)): ee_dbg = ee_dbg * S_FW_dbg[int(elt[i])]
    w_pw0_dbg = setup['PW0'](ee_dbg)
    tv_dbg, fp_dbg = w_pw0_dbg.value
    ...
    if elt:
        p_col, p_lab, drawn_w, _ = _r3_plot_colored_alcove_a3(
            p_col, p_lab, drawn_w, fin_weyl, trans_vec, (0.9,0.9,0.9), 'blue', bbox, alcove_verts, rw)
    # Draw wireframe
    crv_euclid = []
...
def _bc3_conjugacy_class_plot(letter, W_fin, B, H0dict, bbox):
    ...
    _fw_mp_bc = H0dict[fin_weyl].get('mod_plot',[vector([0,0,0])])
    _trans_B_bc = H0dict[fin_weyl].get('trans_B', vector([0,0,0]))
    ...
    def _fmt_b(x):
        ...
        f = float(x)
        return str(int(f)) if f == int(f) else f"{f:.10g}"
    print(f"Basis B (columns = simple coroots of {letter}3):")
    for _i in range(3):
        print(f"  b{_i+1} = ({', '.join([_fmt_b(B[_j,_i]) for _j in range(3)])})")
    print("Translation vectors are in basis B coordinates (n1, n2, n3)")
    _W_p = WeylGroup(["A",3,1]); _S_p = _W_p.simple_reflections()
...
def _conjugacy_class_BC3(letter, elt, bbox):
    ...
    if elt:
        ...
        if _r3_is_in_bounds(chosen_tv, bbox):
            ...
            for i in range(4):
                for j in range(i+1,4): p_col += line3d([chosen_tv[i],chosen_tv[j]],color='blue',thickness=4)
            centroid = np.mean([list(v) for v in chosen_tv],axis=0)
            p_lab += text3d(_r3_format_label(rw),tuple(centroid),color='blue',fontsize=24)
    for tB in crv_in_B:
        ta = ambient.zero()
        ...
        for w in W_fin:
            ...
            for i in range(4):
                for j in range(i+1,4): p_wire += line3d([tv[i],tv[j]],color='#888888',thickness=1)
    G_conj = _bc3_conjugacy_class_plot(letter, W_fin, B, H0dict, bbox)
    # Print conjugacy class elements using PW0.from_translation (original notebook approach)
...
def affine2_conjugacy_class(letter, elt, bbox=10):
    """Conjugacy class in a rank-2 affine Weyl group."""
    letter = letter.upper()
...
def affine2_coconjugation(letter, elt1, elt2, bbox=7):
    """Coconjugation in a rank-2 affine Weyl group."""
    letter = letter.upper()
...
# Set global output dir so rank-3 functions can save 3D plots directly
