...
def _a1xa1_span_vectors(vertex1, vertex2, B, H0_alcoves_dict, coroot_vertices_in_B):
    """Expose mod-set generators for each of the 4 keys of Ã₁×Ã₁.
    Also keeps mod_plot for backward compatibility with any legacy callers."""
    v1B = B.inverse()*vertex1; v2B = B.inverse()*vertex2
    VS = ZZ**2
    # Generators of the mod-set sublattice for each alcove key
    gens = [
        [],                       # identity
        [2*v1B],                  # s1
        [2*v2B],                  # s2
    ...
    # Legacy mod_plot (kept for backward compat with any code that reads it)
    ...
    for i,k in enumerate(H0_alcoves_dict.keys()):
        H0_alcoves_dict[k]['mod_generators'] = gens[i]
        H0_alcoves_dict[k]['mod_plot'] = ms[i]
    return H0_alcoves_dict


def _a1xa1_reflect_cc(trans_part, cp_tmp, H0_alcoves_dict, B=None, bounding_box=None):
    """Compute reflected mod_set points for each Ã₁×Ã₁ alcove key.
    
    If B and bounding_box are provided, enumerate lattice points of the affine sublattice
    directly (avoids missing points due to initial cube truncation).
    Otherwise, fall back to the legacy point-by-point reflection of mod_plot.
    """
    # Reflection matrices for Ã₁×Ã₁: 4 sign-flip combinations
    matrices = [
        [[1,0],[0,1]],     # identity
        [[-1,0],[0,1]],    # s1 (flip x)
        [[1,0],[0,-1]],    # s2 (flip y)
    ...
    # New path: enumerate the affine sublattice directly within the bbox
    if B is not None and bounding_box is not None:
        old_gens = H0_alcoves_dict[cp_tmp].get('mod_generators', [])
        for i, k in enumerate(H0_alcoves_dict.keys()):
            mat = matrix(ZZ, matrices[i])
            new_base = vector(mat * vector(trans_part))
            new_gens = [vector(mat * vector(g)) for g in old_gens]
            pts = _enumerate_affine_sublattice_2d(new_base, new_gens, B, bounding_box)
            H0_alcoves_dict[k]['reflect'] = pts
        return H0_alcoves_dict
    # Legacy path: reflect mod_plot points (can miss points outside the cube)
...
def _Cox_Element_A1xA1(elt_1, elt_2, bounding_box):
    ...
    H0 = _a1xa1_reflect_cc(tp, cp_tmp, H0, B, bounding_box)
...
def _aff2_get_geometry(affine_letter, S_weyl, identity):
    ...
    if affine_letter == 'G':
        vertex1 = vector(AA, [-9/2, 3*sqrt(AA(3))/2])
        vertex2 = vector(AA, [3, 0])
        vertexa = vector(AA, [0, 3*sqrt(AA(3))])
        vertexb = vector(AA, [3/2, 3*sqrt(AA(3))/2])
        ...
        vertex1 = vector(AA, [3/2, sqrt(AA(3))/2])
        vertex2 = vector(AA, [-3/2, sqrt(AA(3))/2])
        vertexa = vector(AA, [1/2, sqrt(AA(3))/2])
        vertexb = vector(AA, [-1/2, sqrt(AA(3))/2])
        ...
        vertex1 = vector(AA, [1,-1])
        vertex2 = vector(AA, [0,2])
        vertexa = vector(AA, [1,0])
        vertexb = vector(AA, [1,1])
        ...
        vertex1 = vector(AA, [2,-2])
        vertex2 = vector(AA, [0,2])
        vertexa = vector(AA, [2,0])
        vertexb = vector(AA, [1,1])
...
def _enumerate_affine_sublattice_2d(base_B, generators_B, B, bounding_box):
    """Enumerate lattice points of affine sublattice {base + sum(n_i * g_i) : n_i integer}
    whose Euclidean position (B * point) is within [-bbox, bbox] in sup-norm.
    
    Uses Sage's Polyhedron.integral_points() to enumerate exactly - no wasted iterations.
    All arithmetic uses Sage's exact symbolic/rational types (no floats).
    
    base_B: translation in B-coordinates (vector of length 2)
    generators_B: list of generator vectors in B-coordinates (0, 1, or 2 generators)
    B: 2x2 matrix converting B-coords to Euclidean
    bounding_box: sup-norm bound on Euclidean coords
    
    Returns list of integer-coordinate vectors (in B-coords) satisfying the bound.
    """
    base_B = vector(base_B)
    if len(generators_B) == 0:
        # Just the base point; check if in bbox
        ec = B * base_B
        if abs(ec[0]) <= bounding_box and abs(ec[1]) <= bounding_box:
            return [vector(base_B)]
        return []
    # Work in "box space" (Euclidean) for the inequalities, since the constraint is
    # that (B * point) lies in [-bounding_box, bounding_box]^2.
    eff_offset = B * base_B
    eff_basis = [B * vector(g) for g in generators_B]
    # Build inequalities for integral_points: each row starts with the constant term
    # and is followed by coefficients for each generator variable.
    ieqs = []
    for i in range(2):
        # -bounding_box <= (B*point)[i] <= bounding_box
        ieqs.append([eff_offset[i] + bounding_box] + [b[i] for b in eff_basis])
        ieqs.append([bounding_box - eff_offset[i]] + [-b[i] for b in eff_basis])
    P = Polyhedron(ieqs=ieqs)
    coeffs = P.integral_points()  # integer (n_1, ..., n_k) tuples
    # Convert back to B-coordinate points
    return [vector(base_B + sum(n*g for n, g in zip(c, generators_B))) for c in coeffs]


def _enumerate_affine_sublattice_3d(base_B, generators_B, B, bounding_box):
    """3D version of _enumerate_affine_sublattice_2d.
    
    Works identically but in 3 Euclidean dimensions. The generators_B list can have
    0, 1, 2, or 3 entries (for rank-0 through rank-3 sublattices).
    
    base_B: translation in B-coordinates (vector of length 3)
    generators_B: list of generator vectors in B-coordinates
    B: 3x3 matrix converting B-coords to Euclidean
    bounding_box: sup-norm bound on Euclidean coords
    ...
    if len(generators_B) == 0:
        ec = B * base_B
        if abs(ec[0]) <= bounding_box and abs(ec[1]) <= bounding_box and abs(ec[2]) <= bounding_box:
            return [vector(base_B)]
        return []
    eff_offset = B * base_B
    eff_basis = [B * vector(g) for g in generators_B]
...
def _aff2_mod_set(letter, v1, v2, B, H0dict, crv_in_B, cp_tmp, keys):
    ...
    VS = ZZ**2
    if letter == 'C':
        # Store generators as lists; identity has no generators (single point at origin)
        gens = {keys['identity']:[],
                keys['s1']:[v1B], keys['s2']:[2*v2B],
                keys['s12']:[v1B,2*v2B], keys['s21']:[v1B,2*v2B],
                keys['s121']:[2*v1B+2*v2B],
                keys['s212']:[v1B+2*v2B],
                keys['s2121']:[2*v1B,2*v2B]}
        ...
        gens = {keys['identity']:[],
                keys['s1']:[v1B], keys['s2']:[v2B],
                keys['s12']:[v1B+2*v2B,3*v2B],
                keys['s21']:[v1B+2*v2B,3*v2B],
                keys['s212']:[v1B+v2B],
                keys['s121']:[v1B+v2B]}
        spans = {keys['identity']:[vector([0,0])],
        ...
        gens = {keys['identity']:[],
                keys['s1']:[2*v1B], keys['s2']:[v2B],
                keys['s12']:[v1B,v2B], keys['s21']:[2*v1B,v2B],
                keys['s121']:[2*v1B+v2B],
                keys['s212']:[2*v1B+2*v2B],
                keys['s2121']:[2*v1B,2*v2B]}
        spans = {keys['identity']:[vector([0,0])],
        ...
        gens = {keys['identity']:[],
                keys['s1']:[v1B], keys['s2']:[v2B],
                keys['s12']:[v1B,v2B], keys['s21']:[v1B,v2B],
                keys['s121']:[v1B+v2B],
                keys['s212']:[v1B+3*v2B],
                keys['s1212']:[v1B,3*v2B],
                keys['s2121']:[v1B,3*v2B],
                keys['s12121']:[2*v1B+3*v2B],
                keys['s21212']:[v1B+2*v2B],
                keys['s212121']:[2*v1B,2*v2B]}
        spans = {keys['identity']:[vector([0,0])],
                 keys['s1']:VS.span([v1B]), keys['s2']:VS.span([v2B]),
    ...
    # Store generators for later use in _aff2_reflect_cc
    for k in H0dict:
        if k in gens:
            H0dict[k]['mod_generators'] = gens[k]
    # Also keep old 'mod_set' for backward compatibility (points of span within crv_in_B)
    mod_plots = {}
...
def _aff2_reflect_cc(letter, tp, cp_tmp, H0dict, keys, B=None, bounding_box=None):
    """Compute reflected mod_set points for each alcove key, filtered by bounding box.
    
    If B and bounding_box are provided, enumerate lattice points of the affine sublattice
    directly (avoids missing points due to initial crv_in_B truncation).
    Otherwise, fall back to the legacy point-by-point reflection of mod_set.
    """
    # Reflection matrices for each letter: dict mapping key -> 2x2 matrix (as list of rows)
    if letter == 'G':
        M = {keys['identity']:[[1,0],[0,1]],
             keys['s1']:[[-1,1],[0,1]],
             keys['s2']:[[1,0],[3,-1]],
             keys['s12']:[[2,-1],[3,-1]],
             keys['s21']:[[-1,1],[-3,2]],
    ...
    if B is not None and bounding_box is not None:
        for k in H0dict:
            ...
            pts = _enumerate_affine_sublattice_2d(new_base, new_gens, B, bounding_box)
            H0dict[k]['reflect'] = pts
        return H0dict
    # Legacy path: use mod_set points (can miss points outside crv_in_B)
...
def _conjugacy_class_affine2(letter, elt, bounding_box):
    ...
    _aff2_reflect_cc(letter, tp, cp_tmp, H0dict, keys, B, bounding_box)
...
def _a3_to_3d(vector_4_dim):
    v = vector(AA, vector_4_dim)
    v = v - sum(v)/AA(4) * vector(AA, [1,1,1,1])
    e1 = vector(AA,[1,-1,0,0])/sqrt(AA(2))
    e2 = vector(AA,[1,1,-2,0])/sqrt(AA(6))
    e3 = vector(AA,[1,1,1,-3])/sqrt(AA(12))
    return vector(AA,[v.dot_product(e1),v.dot_product(e2),v.dot_product(e3)])
...
def _a3_setup():
    ...
    coroots_3d = [vector(AA, _a3_to_3d(list(cr.to_vector()))) for cr in simple_coroots]
    B = matrix(AA, coroots_3d).transpose()
    fund_weights = [ambient.fundamental_weight(i) for i in [1,2,3]]
    fw_3d = [vector(AA, _a3_to_3d(list(fw.to_vector()))) for fw in fund_weights]
    C = matrix(AA, fw_3d).transpose()
    origin = ambient.zero()
...
def _a3_mod_set(cp, crv_in_B, setup):
    ...
    # Mod-set generators for each finite Weyl element (key).
    # These are the generators of the sublattice {image(k - I) ∩ Q^v} 
    # used in the conjugacy class enumeration.
    mod_gens = {
        'identity':[],
        's1':[vector([1,0,0])], 's2':[vector([0,1,0])],
        's3':[vector([0,0,1])],
        's12':[vector([1,0,0]),vector([0,1,0])],
        's21':[vector([1,0,0]),vector([0,1,0])],
        's23':[vector([0,1,0]),vector([0,0,1])],
        's32':[vector([0,1,0]),vector([0,0,1])],
        's31':[vector([1,0,1]),vector([0,0,2])],
        's121':[vector([1,1,0])], 's232':[vector([0,1,1])],
        's123':[vector([1,0,3]),vector([0,1,3]),vector([0,0,4])],
        's321':[vector([1,0,3]),vector([0,1,3]),vector([0,0,4])],
        's231':[vector([1,0,1]),vector([0,1,2]),vector([0,0,4])],
        's312':[vector([1,0,1]),vector([0,1,2]),vector([0,0,4])],
        's1231':[vector([1,1,0]),vector([0,0,1])],
        's3121':[vector([1,1,0]),vector([0,0,1])],
        's1232':[vector([1,0,0]),vector([0,1,1])],
        's2321':[vector([1,0,0]),vector([0,1,1])],
        's2312':[vector([1,0,-1]),vector([0,2,2])],
        's12312':[vector([1,0,1]),vector([0,2,1]),vector([0,0,2])],
        's23121':[vector([1,0,1]),vector([0,2,1]),vector([0,0,2])],
        's12321':[vector([1,1,1])],
        's123121':[vector([1,0,1]),vector([0,2,0])],
    }
    # Legacy mod_spans (kept for backward compat with crv_in_B-based filtering)
    mod_spans = {k: (VS.span(v) if v else [vector([0,0,0])]) for k,v in mod_gens.items()}
...
def _a3_mod_generators(cp):
    """Returns the generators (as list of B-coord vectors) of the mod-set sublattice
    for alcove key cp in Ã₃. Used by the new _enumerate_affine_sublattice_3d path."""
    kn = _a3_get_keyname(cp)
    mod_gens = {
        'identity':[],
        's1':[vector([1,0,0])], 's2':[vector([0,1,0])],
        's3':[vector([0,0,1])],
    ...
    return mod_gens.get(kn, [])


def _a3_reflection_matrix(kn):
    """Returns the 3x3 reflection matrix (in B-coords) for alcove key kn in Ã₃.
    These are hardcoded from the reflection formulas in _a3_conjugacy_class_plot,
    where for vector v = (a,b,c), the reflected vector is R_k · v."""
    M = {
        'identity':[[1,0,0],[0,1,0],[0,0,1]],
        's1':[[-1,1,0],[0,1,0],[0,0,1]],         # (-a+b, b, c)
        's2':[[1,0,0],[1,-1,1],[0,0,1]],         # (a, a-b+c, c)
        's3':[[1,0,0],[0,1,0],[0,1,-1]],         # (a, b, b-c)
        's12':[[0,-1,1],[1,-1,1],[0,0,1]],       # (-b+c, a-b+c, c)
        's21':[[-1,1,0],[-1,0,1],[0,0,1]],       # (-a+b, -a+c, c)
        's23':[[1,0,0],[1,0,-1],[0,1,-1]],       # (a, a-c, b-c)
        's31':[[-1,1,0],[0,1,0],[0,1,-1]],       # (-a+b, b, b-c)
        's32':[[1,0,0],[1,-1,1],[1,-1,0]],       # (a, a-b+c, a-b)
        's121':[[0,-1,1],[-1,0,1],[0,0,1]],      # (-b+c, -a+c, c)
        's123':[[0,0,-1],[1,0,-1],[0,1,-1]],     # (-c, a-c, b-c)
        's231':[[-1,1,0],[-1,1,-1],[0,1,-1]],    # (-a+b, -a+b-c, b-c)
        's232':[[1,0,0],[1,0,-1],[1,-1,0]],      # (a, a-c, a-b)
        's312':[[0,-1,1],[1,-1,1],[1,-1,0]],     # (-b+c, a-b+c, a-b)
        's321':[[-1,1,0],[-1,0,1],[-1,0,0]],     # (-a+b, -a+c, -a)
        's1231':[[0,0,-1],[-1,1,-1],[0,1,-1]],   # (-c, -a+b-c, b-c)
        's1232':[[0,0,-1],[1,0,-1],[1,-1,0]],    # (-c, a-c, a-b)
        's2312':[[0,-1,1],[0,-1,0],[1,-1,0]],    # (-b+c, -b, a-b)
        's2321':[[-1,1,0],[-1,1,-1],[-1,0,0]],   # (-a+b, -a+b-c, -a)
        's3121':[[0,-1,1],[-1,0,1],[-1,0,0]],    # (-b+c, -a+c, -a)
        's12312':[[0,0,-1],[0,-1,0],[1,-1,0]],   # (-c, -b, a-b)
        's12321':[[0,0,-1],[-1,1,-1],[-1,0,0]],  # (-c, -a+b-c, -a)
        's23121':[[0,-1,1],[0,-1,0],[-1,0,0]],   # (-b+c, -b, -a)
        's123121':[[0,0,-1],[0,-1,0],[-1,0,0]],  # (-c, -b, -a)
    }
    return matrix(ZZ, M.get(kn, [[1,0,0],[0,1,0],[0,0,1]]))
...
def _a3_conjugacy_class_plot(W_fin, B, H0dict, bbox, alcove_verts, trans_B, fin_weyl):
    ...
    # New approach: for each key k, use hardcoded reflection matrix applied to
    # (trans_B + span(mod_generators)), then enumerate lattice points tightly.
    # This avoids the legacy crv_in_B-based filtering which could miss points.
    fw_mod_gens = _a3_mod_generators(fin_weyl)
    for k in H0dict.keys():
        kn = _a3_get_keyname(k)
        mat = _a3_reflection_matrix(kn)
        new_base = vector(mat * vector(trans_B))
        new_gens = [vector(mat * vector(g)) for g in fw_mod_gens]
        pts = _enumerate_affine_sublattice_3d(new_base, new_gens, B, bbox)
        H0dict[k]['reflect'] = pts
...
def _bc3_mod_generators(cp, letter):
    """Returns the generators (as list of B-coord vectors) of the mod-set sublattice
    for alcove key cp in B̃₃ or C̃₃."""
    kn = _bc3_get_keyname(cp)
    if letter == 'B':
        gens_dict = {
        'identity':[],
        's1':[vector([1, 0, 0])],
        ...
        's31':[vector([1, 0, 1]),vector([0, 0, 2])],
        's121':[vector([1, 1, 0])],
        's123':[vector([1, 0, 1]),vector([0, 1, 1]),vector([0, 0, 2])],
        's232':[vector([0, 2, 1])],
        's231':[vector([1, 0, 1]),vector([0, 1, 0]),vector([0, 0, 2])],
        's321':[vector([1, 0, 1]),vector([0, 1, 1]),vector([0, 0, 2])],
        's312':[vector([1, 0, 1]),vector([0, 1, 0]),vector([0, 0, 2])],
        's323':[vector([0, 1, 1])],
        's1231':[vector([1, 1, 0]),vector([0, 0, 1])],
        's1232':[vector([1, 0, 0]),vector([0, 2, 1])],
        's2312':[vector([1, 3, 1]),vector([0, 4, 2])],
        's2321':[vector([1, 0, 0]),vector([0, 2, 1])],
        's3121':[vector([1, 1, 0]),vector([0, 0, 1])],
        's3123':[vector([1, 0, 0]),vector([0, 1, 1])],
        's3231':[vector([1, 0, 0]),vector([0, 1, 1])],
        's3232':[vector([0, 2, 0]),vector([0, 0, 1])],
        's12312':[vector([1, 0, 0]),vector([0, 1, 1]),vector([0, 0, 2])],
        's12321':[vector([2, 2, 1])],
        's23121':[vector([1, 0, 0]),vector([0, 1, 1]),vector([0, 0, 2])],
        's23123':[vector([1, 3, 0]),vector([0, 4, 0]),vector([0, 0, 1])],
        's31231':[vector([1, 1, 1])],
        's31232':[vector([1, 0, 1]),vector([0, 2, 1]),vector([0, 0, 2])],
        's32312':[vector([1, 3, 0]),vector([0, 4, 0]),vector([0, 0, 1])],
        's32321':[vector([1, 0, 1]),vector([0, 2, 1]),vector([0, 0, 2])],
        's123121':[vector([2, 1, 1]),vector([0, 2, 0])],
        's231231':[vector([1, 0, 1]),vector([0, 1, 0])],
        's231232':[vector([1, 0, -1]),vector([0, 1, 1])],
        's312312':[vector([1, 0, 1]),vector([0, 1, 0])],
        's312321':[vector([2, 2, 0]),vector([0, 0, 1])],
        's323121':[vector([1, 0, -1]),vector([0, 1, 1])],
        's323123':[vector([1, 3, 2]),vector([0, 4, 2])],
        's3123121':[vector([2, 1, 0]),vector([0, 2, 0]),vector([0, 0, 1])],
        's3231231':[vector([1, 0, 0]),vector([0, 1, 0]),vector([0, 0, 2])],
        's3231232':[vector([1, 0, 0]),vector([0, 1, 0]),vector([0, 0, 2])],
        's2312312':[vector([1, 2, 1])],
        's2312321':[vector([2, 1, 0]),vector([0, 2, 0]),vector([0, 0, 1])],
        's23123121':[vector([2, 0, 0]),vector([0, 2, 1])],
        's32312312':[vector([1, 2, 0]),vector([0, 0, 2])],
        's32312321':[vector([2, 1, 0]),vector([0, 2, 2])],
        's323123121':[vector([2, 0, 0]),vector([0, 2, 0]),vector([0, 0, 2])]
        }
    else:  # C
        gens_dict = {
        'identity':[],
        ...
        's12':[vector([1, 0, 0]),vector([0, 1, 0])],
        's21':[vector([1, 0, 0]),vector([0, 1, 0])],
        's23':[vector([0, 1, 0]),vector([0, 0, 2])],
        's32':[vector([0, 1, 0]),vector([0, 0, 2])],
        's31':[vector([1, 0, 0]),vector([0, 0, 2])],
        's121':[vector([1, 1, 0])],
        's123':[vector([1, 0, 0]),vector([0, 1, 0]),vector([0, 0, 2])],
        's232':[vector([0, 2, 2])],
        's231':[vector([1, 0, 0]),vector([0, 1, 0]),vector([0, 0, 2])],
        's321':[vector([1, 0, 0]),vector([0, 1, 0]),vector([0, 0, 2])],
        's312':[vector([1, 0, 0]),vector([0, 1, 0]),vector([0, 0, 2])],
        's323':[vector([0, 1, 2])],
        's1231':[vector([1, 1, 0]),vector([0, 0, 2])],
        's1232':[vector([1, 0, 0]),vector([0, 2, 2])],
        's2312':[vector([1, 1, 0]),vector([0, 2, 2])],
        's2321':[vector([1, 0, 0]),vector([0, 2, 2])],
        's3121':[vector([1, 1, 0]),vector([0, 0, 2])],
        's3123':[vector([1, 0, 0]),vector([0, 1, 2])],
        's3231':[vector([1, 0, 0]),vector([0, 1, 2])],
        's3232':[vector([0, 2, 0]),vector([0, 0, 2])],
        's12312':[vector([1, 0, 0]),vector([0, 1, 0]),vector([0, 0, 2])],
        's12321':[vector([2, 2, 2])],
        's23121':[vector([1, 0, 0]),vector([0, 1, 0]),vector([0, 0, 2])],
        's23123':[vector([1, 1, 0]),vector([0, 2, 0]),vector([0, 0, 2])],
        's31231':[vector([1, 1, 2])],
        's31232':[vector([1, 0, 0]),vector([0, 2, 0]),vector([0, 0, 2])],
        's32312':[vector([1, 1, 0]),vector([0, 2, 0]),vector([0, 0, 2])],
        's32321':[vector([1, 0, 0]),vector([0, 2, 0]),vector([0, 0, 2])],
        's123121':[vector([2, 0, 2]),vector([0, 1, 0])],
        's231231':[vector([1, 0, 2]),vector([0, 1, 0])],
        's231232':[vector([1, 0, -2]),vector([0, 1, 2])],
        's312312':[vector([1, 0, 2]),vector([0, 1, 0])],
        's312321':[vector([2, 2, 0]),vector([0, 0, 2])],
        's323121':[vector([1, 0, -2]),vector([0, 1, 2])],
        's323123':[vector([1, 1, 2]),vector([0, 2, 2])],
        's3123121':[vector([2, 0, 0]),vector([0, 1, 0]),vector([0, 0, 2])],
        's3231231':[vector([1, 0, 0]),vector([0, 1, 0]),vector([0, 0, 2])],
        's3231232':[vector([1, 0, 0]),vector([0, 1, 0]),vector([0, 0, 2])],
        's2312312':[vector([1, 2, 2])],
        's2312321':[vector([2, 0, 0]),vector([0, 1, 0]),vector([0, 0, 2])],
        's23123121':[vector([2, 0, 0]),vector([0, 2, 2])],
        's32312312':[vector([1, 2, 0]),vector([0, 0, 2])],
        's32312321':[vector([2, 0, -2]),vector([0, 1, 2])],
        's323123121':[vector([2, 0, 0]),vector([0, 2, 0]),vector([0, 0, 2])]
        }
    return gens_dict.get(kn, [])


def _bc3_reflection_matrix(kn, letter):
    """Returns the 3x3 reflection matrix (in B-coords) for alcove key kn in B̃₃ or C̃₃.
    These are hardcoded from the reflection formulas in _b3_reflect / _c3_reflect."""
    if letter == 'B':
        M = {
        'identity':[[1, 0, 0],[0, 1, 0],[0, 0, 1]],
        's1':[[-1, 1, 0],[0, 1, 0],[0, 0, 1]],
        's2':[[1, 0, 0],[1, -1, 2],[0, 0, 1]],
        's3':[[1, 0, 0],[0, 1, 0],[0, 1, -1]],
        's12':[[0, -1, 2],[1, -1, 2],[0, 0, 1]],
        's21':[[-1, 1, 0],[-1, 0, 2],[0, 0, 1]],
        's23':[[1, 0, 0],[1, 1, -2],[0, 1, -1]],
        's31':[[-1, 1, 0],[0, 1, 0],[0, 1, -1]],
        's32':[[1, 0, 0],[1, -1, 2],[1, -1, 1]],
        's121':[[0, -1, 2],[-1, 0, 2],[0, 0, 1]],
        's123':[[0, 1, -2],[1, 1, -2],[0, 1, -1]],
        's231':[[-1, 1, 0],[-1, 2, -2],[0, 1, -1]],
        's232':[[1, 0, 0],[2, -1, 0],[1, -1, 1]],
        's312':[[0, -1, 2],[1, -1, 2],[1, -1, 1]],
        's321':[[-1, 1, 0],[-1, 0, 2],[-1, 0, 1]],
        's323':[[1, 0, 0],[1, 1, -2],[1, 0, -1]],
        's1231':[[0, 1, -2],[-1, 2, -2],[0, 1, -1]],
        's1232':[[1, -1, 0],[2, -1, 0],[1, -1, 1]],
        's2312':[[0, -1, 2],[1, -2, 2],[1, -1, 1]],
        's2321':[[-1, 1, 0],[-2, 1, 0],[-1, 0, 1]],
        's3121':[[0, -1, 2],[-1, 0, 2],[-1, 0, 1]],
        's3123':[[0, 1, -2],[1, 1, -2],[1, 0, -1]],
        's3231':[[-1, 1, 0],[-1, 2, -2],[-1, 1, -1]],
        's3232':[[1, 0, 0],[2, -1, 0],[1, 0, -1]],
        's12312':[[1, -1, 0],[1, -2, 2],[1, -1, 1]],
        's12321':[[-1, 0, 0],[-2, 1, 0],[-1, 0, 1]],
        's23121':[[0, -1, 2],[-1, -1, 2],[-1, 0, 1]],
        's23123':[[0, 1, -2],[1, 0, -2],[1, 0, -1]],
        's31231':[[0, 1, -2],[-1, 2, -2],[-1, 1, -1]],
        's31232':[[1, -1, 0],[2, -1, 0],[1, 0, -1]],
        's32312':[[0, -1, 2],[1, -2, 2],[0, -1, 1]],
        's32321':[[-1, 1, 0],[-2, 1, 0],[-1, 1, -1]],
        's123121':[[-1, 0, 0],[-1, -1, 2],[-1, 0, 1]],
        's231231':[[0, 1, -2],[-1, 1, -2],[-1, 1, -1]],
        's231232':[[1, -1, 0],[1, 0, -2],[1, 0, -1]],
        's312312':[[1, -1, 0],[1, -2, 2],[0, -1, 1]],
        's312321':[[-1, 0, 0],[-2, 1, 0],[-1, 1, -1]],
        's323121':[[0, -1, 2],[-1, -1, 2],[0, -1, 1]],
        's323123':[[0, 1, -2],[1, 0, -2],[0, 0, -1]],
        's2312312':[[1, -1, 0],[0, -1, 0],[0, -1, 1]],
        's2312321':[[-1, 0, 0],[-1, 1, -2],[-1, 1, -1]],
        's3123121':[[-1, 0, 0],[-1, -1, 2],[0, -1, 1]],
        's3231231':[[0, 1, -2],[-1, 1, -2],[0, 0, -1]],
        's3231232':[[1, -1, 0],[1, 0, -2],[0, 0, -1]],
        's23123121':[[-1, 0, 0],[0, -1, 0],[0, -1, 1]],
        's32312312':[[1, -1, 0],[0, -1, 0],[0, 0, -1]],
        's32312321':[[-1, 0, 0],[-1, 1, -2],[0, 0, -1]],
        's323123121':[[-1, 0, 0],[0, -1, 0],[0, 0, -1]]
        }
    else:  # C
        M = {
        'identity':[[1, 0, 0],[0, 1, 0],[0, 0, 1]],
        's1':[[-1, 1, 0],[0, 1, 0],[0, 0, 1]],
        's2':[[1, 0, 0],[1, -1, 1],[0, 0, 1]],
        's3':[[1, 0, 0],[0, 1, 0],[0, 2, -1]],
        's12':[[0, -1, 1],[1, -1, 1],[0, 0, 1]],
        's21':[[-1, 1, 0],[-1, 0, 1],[0, 0, 1]],
        's23':[[1, 0, 0],[1, 1, -1],[0, 2, -1]],
        's31':[[-1, 1, 0],[0, 1, 0],[0, 2, -1]],
        's32':[[1, 0, 0],[1, -1, 1],[2, -2, 1]],
        's121':[[0, -1, 1],[-1, 0, 1],[0, 0, 1]],
        's123':[[0, 1, -1],[1, 1, -1],[0, 2, -1]],
        's231':[[-1, 1, 0],[-1, 2, -1],[0, 2, -1]],
        's232':[[1, 0, 0],[2, -1, 0],[2, -2, 1]],
        's312':[[0, -1, 1],[1, -1, 1],[2, -2, 1]],
        's321':[[-1, 1, 0],[-1, 0, 1],[-2, 0, 1]],
        's323':[[1, 0, 0],[1, 1, -1],[2, 0, -1]],
        's1231':[[0, 1, -1],[-1, 2, -1],[0, 2, -1]],
        's1232':[[1, -1, 0],[2, -1, 0],[2, -2, 1]],
        's2312':[[0, -1, 1],[1, -2, 1],[2, -2, 1]],
        's2321':[[-1, 1, 0],[-2, 1, 0],[-2, 0, 1]],
        's3121':[[0, -1, 1],[-1, 0, 1],[-2, 0, 1]],
        's3123':[[0, 1, -1],[1, 1, -1],[2, 0, -1]],
        's3231':[[-1, 1, 0],[-1, 2, -1],[-2, 2, -1]],
        's3232':[[1, 0, 0],[2, -1, 0],[2, 0, -1]],
        's12312':[[1, -1, 0],[1, -2, 1],[2, -2, 1]],
        's12321':[[-1, 0, 0],[-2, 1, 0],[-2, 0, 1]],
        's23121':[[0, -1, 1],[-1, -1, 1],[-2, 0, 1]],
        's23123':[[0, 1, -1],[1, 0, -1],[2, 0, -1]],
        's31231':[[0, 1, -1],[-1, 2, -1],[-2, 2, -1]],
        's31232':[[1, -1, 0],[2, -1, 0],[2, 0, -1]],
        's32312':[[0, -1, 1],[1, -2, 1],[0, -2, 1]],
        's32321':[[-1, 1, 0],[-2, 1, 0],[-2, 2, -1]],
        's123121':[[-1, 0, 0],[-1, -1, 1],[-2, 0, 1]],
        's231231':[[0, 1, -1],[-1, 1, -1],[-2, 2, -1]],
        's231232':[[1, -1, 0],[1, 0, -1],[2, 0, -1]],
        's312312':[[1, -1, 0],[1, -2, 1],[0, -2, 1]],
        's312321':[[-1, 0, 0],[-2, 1, 0],[-2, 2, -1]],
        's323121':[[0, -1, 1],[-1, -1, 1],[0, -2, 1]],
        's323123':[[0, 1, -1],[1, 0, -1],[0, 0, -1]],
        's2312312':[[1, -1, 0],[0, -1, 0],[0, -2, 1]],
        's2312321':[[-1, 0, 0],[-1, 1, -1],[-2, 2, -1]],
        's3123121':[[-1, 0, 0],[-1, -1, 1],[0, -2, 1]],
        's3231231':[[0, 1, -1],[-1, 1, -1],[0, 0, -1]],
        's3231232':[[1, -1, 0],[1, 0, -1],[0, 0, -1]],
        's23123121':[[-1, 0, 0],[0, -1, 0],[0, -2, 1]],
        's32312312':[[1, -1, 0],[0, -1, 0],[0, 0, -1]],
        's32312321':[[-1, 0, 0],[-1, 1, -1],[0, 0, -1]],
        's323123121':[[-1, 0, 0],[0, -1, 0],[0, 0, -1]]
        }
    return matrix(ZZ, M.get(kn, [[1,0,0],[0,1,0],[0,0,1]]))
...
def _bc3_conjugacy_class_plot(letter, W_fin, B, H0dict, bbox, fin_weyl=None):
    ...
    fw_mod_gens = _bc3_mod_generators(_fw_ref, letter)
    for k in H0dict.keys():
        kn = _bc3_get_keyname(k)
        mat = _bc3_reflection_matrix(kn, letter)
        new_base = vector(mat * vector(_trans_B_bc))
        new_gens = [vector(mat * vector(g)) for g in fw_mod_gens]
        pts = _enumerate_affine_sublattice_3d(new_base, new_gens, B, bbox)
        H0dict[k]['reflect'] = pts
